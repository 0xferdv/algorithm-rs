...
impl<K: Ord, V> RBNode<K, V> {
    ...
    ///
    /// # 返回值
    /// 返回新创建的RBNode实例，颜色默认为红色，所有指针初始化为空
...
impl<K: Ord, V> Default for RBTree<K, V> {
    /// 为RBTree实现Default trait，提供默认构造方法
    ///
    /// # 返回值
    /// 返回空的红黑树实例
    fn default() -> Self {
