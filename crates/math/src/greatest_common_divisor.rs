...
/**
 * 计算两个整数的最大公约数（GCD），使用递归实现的欧几里得算法。
 *
 * 参数:
 * a: 第一个整数
 * b: 第二个整数
...
/**
 * 计算两个整数的最大公约数（GCD），使用迭代实现的欧几里得算法。
 *
 * 参数:
 * a: 第一个整数
 * b: 第二个整数
 ...
 */
#[allow(unused)]
pub fn greatest_common_divisor_recursive(mut a: i64, mut b: i64) -> i64 {
    // 使用欧几里得算法迭代计算最大公约数
    while a != 0 {
        let remainder = b % a;
        b = a;
...
/**
 * 使用Stein算法（二进制GCD算法）计算两个无符号整数的最大公约数。
 *
 * 参数:
 * a: 第一个无符号整数
 * b: 第二个无符号整数
...
mod tests {
    ...
    fn positive_number_iterative() {
        assert_eq!(greatest_common_divisor(4, 16), 4);
        assert_eq!(greatest_common_divisor(16, 4), 4);
        assert_eq!(greatest_common_divisor(3, 5), 1);
        assert_eq!(greatest_common_divisor(40, 40), 40);
        assert_eq!(greatest_common_divisor(27, 12), 3);
    }
    ...
    fn negative_number_iterative() {
        assert_eq!(greatest_common_divisor(-32, -8), 8);
        assert_eq!(greatest_common_divisor(-8, -32), 8);
        assert_eq!(greatest_common_divisor(-3, -5), 1);
        assert_eq!(greatest_common_divisor(-40, -40), 40);
        assert_eq!(greatest_common_divisor(-12, -27), 3);
    }
    ...
    fn mix_iterative() {
        ...
        assert_eq!(greatest_common_divisor(-64, 32), 32);
        assert_eq!(greatest_common_divisor(-32, 64), 32);
        assert_eq!(greatest_common_divisor(-40, 40), 40);
        assert_eq!(greatest_common_divisor(12, -27), 3);
    }
}
